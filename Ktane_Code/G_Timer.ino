void timerLoop() {
  if (!MILLISTIMER || timer > 59) {
    if (millisTimer - last_timer_update > 1000 * pow(2, 2 - STRIKES) / (4 - STRIKES)) {   //1000 * 4/4, 2/3, 1/2 //TBU add strike nums again
      timer--;
      if (timer != 0) {
        buzzer(1, 800, 100);
      ...
      last_timer_update = millisTimer;
    }
  }
  else {
    double k = 50;
    if (millisTimer - last_timer_update > (k * pow(2, 2 - STRIKES) / (4 - STRIKES)) - 1) { //1000 * 4/4, 2/3, 1/2 //TBU add strike nums again
